...
For he always argued that he had already written everything in his notebook and that he also duplicated it like one of those doors everyone knows how to open or (if they don't know how) they open it using Arithmetic like in an American movie heist that even I don't remember.`
            ...
            es: [
                {
                    id: "I",
                    text: `Sí, Gregory, sí, es esa cara, el cuerpo sin cara, no, la cara sin cuerpo, la cara que ves incluso en la universidad, sí, sí, Gregory, esa cara que ya no pertenece a esa persona, sino a tu mente. Sí, así es, Gregory.

Y es como si quisieras dejar de verla en la gente, pero aún la ves, es geométrica, sí, de hecho lo es, pero más que eso, bueno, tiene la forma de su cara (sí, la de ella, sí, de hecho es la de ella, Gregory).

Y en la pared, en tinta negra, decía:
El mundo
se desmorona
pero en forma
de cuadrado`,
                ...
                {
                    id: "II",
                    text: `Esa técnica de escribir 1 cuento corto,
luego 1 novela,
y posteriormente 1 comedia,
era una tradición familiar
(...)
Fuentes confiables dicen
(recopilado de los textos de Edmundo en el Nuevo Mundo)
que las pesadillas de Wagner no habían sido más que un encantamiento
del no-demiurgo y los males de la muerte,
y la comedia que no escribió [...]`,
                    audio: "audio/audio2.mp3"
                },
                {
                    id: "III",
...
(que probablemente es una película en blanco y negro)
O simplemente la variante de un discurso mal entregado`,
                    audio: "audio/audio3.mp3"
                },
                {
                    id: "IV",
...
fue negado por el soñador del Cielo
y, después de seis noches,
no hubo registro de fantasías;
los tormentos de tu madre y los gritos de las estrellas
no han envejecido como el vino,
ni en el Infierno ni en nuestros recuerdos.`,
                ...
                },
                {
                    id: "V",
                    text: `
El Cielo podría imaginarse
como un sonido electrónico
o, si hemos de ser justos,
como algo ficticio.
Pero la soledad no puede ser justa,
porque el "Otro" existe (quien lo niega de inmediato)
y es el más codiciado por los hombres:
EL INFIERNO.`,
                    audio: "audio/audio5.mp3"
                },
                {
...
pero si despertamos
ni siquiera sabríamos el nombre.`,
                    audio: "audio/audio6.mp3"
                },
                {
                    id: "VII",
...
For he always argued that he had already written everything in his notebook and that he also duplicated it like one of those doors everyone knows how to open or (if they don't know how) they open it using Arithmetic like in an American movie heist that even I don't remember.`
    ...
    loadQuote(index) {
        try {
            if (index < 0 || index >= this.quotes[this.currentLanguage].length) {
                throw new Error('Quote index out of range');
            }

            const quote = this.quotes[this.currentLanguage][index];
    ...
    updateNavigationButtons() {
        ...
        this.elements.nextBtn.disabled = this.currentQuoteIndex === this.quotes[this.currentLanguage].length - 1;
    }
    ...
    nextQuote() {
        if (this.currentQuoteIndex < this.quotes[this.currentLanguage].length - 1) {
            this.currentQuoteIndex++;
            this.loadQuote(this.currentQuoteIndex);
        }
    }
    ...
    startSpeechSynthesis() {
        ...
        const utterance = new SpeechSynthesisUtterance(this.quotes[this.currentLanguage][this.currentQuoteIndex].text);
        ...
        utterance.onboundary = (event) => {
            if (event.name === 'word') {
                const wordIndex = this.words.findIndex(word => 
                    event.charIndex >= this.quotes[this.currentLanguage][this.currentQuoteIndex].text.indexOf(word) &&
                    event.charIndex < this.quotes[this.currentLanguage][this.currentQuoteIndex].text.indexOf(word) + word.length
                );
                if (wordIndex >= 0) {
                    this.highlightWord(wordIndex);
                    this.updateProgress((wordIndex / this.words.length) * 100);
    ...
    toggleLanguage() {
        ...
        const languageText = this.elements.languageToggle.querySelector('.language-text');
        languageText.textContent = this.currentLanguage.toUpperCase();
        
        // Add a fun animation effect
        this.elements.languageToggle.style.transform = 'scale(0.95)';
        setTimeout(() => {
            this.elements.languageToggle.style.transform = '';
        }, 100);

        // Update UI text and reload current quote
        this.updateUIText();
        this.updateTotalQuotes();
        this.loadQuote(this.currentQuoteIndex);
    }
    ...
    updateUIText() {
        const translations = {
            en: {
                appTitle: 'Supplizios enumerados',
                loading: 'Loading quotes...',
                audioFile: 'Upload a random mp3 if the audio is not playing, buddy',
                highlight: 'Highlight',
                speed: 'Speed'
            },
            es: {
                ...
                audioFile: 'Sube un mp3 random si es que no se escucha el audio, rufiancito',
                highlight: 'Resaltado',
                speed: 'Velocidad'
            }
        };
        ...
        document.querySelector('label[for="audioFile"]').textContent = t.audioFile;
        document.querySelector('label[for="highlightColor"]').textContent = t.highlight;
        document.querySelector('label[for="speechRate"]').textContent = t.speed;
    }
